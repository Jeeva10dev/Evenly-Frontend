...
import { createSettlement } from "../utils/api";
 ...
 * - entityData: object containing data about the user/group balances
 * - currentUserId: string
 * - createSettlement: async function({ amount, note, paidByUserId, receivedByUserId, groupId? })
 * - onSuccess: optional callback
 */
export default function SettlementForm({ entityType, entityData, currentUserId, createSettlement, onSuccess }) {
  ...
  const handleUserSettlement = async (data) => {
    ...
    try {
      await createSettlement({ amount, note: data.note, paidByUserId, receivedByUserId });
  ...
  const handleGroupSettlement = async (data, selectedUserId) => {
    ...
    const paidByUserId =
      data.paymentType === "youPaid" ? currentUserId : selectedUser.userId;
    const receivedByUserId =
      data.paymentType === "youPaid" ? selectedUser.userId : currentUserId;
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Label htmlFor="youPaid" className="flex-grow cursor-pointer">
                <div className="flex items-center">
                  <Avatar className="h-6 w-6 mr-2">
                    <AvatarImage src={entityData.currentUser.imageUrl} />
                    <AvatarFallback>{entityData.currentUser.name.charAt(0)}</AvatarFallback>
                  </Avatar>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input id="amount" placeholder="0.00" type="number" step="0.01" min="0.01" className="pl-7" {...register("amount")} />
          </div>
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`font-medium ${isOwing ? "text-green-600" : isOwed ? "text-red-600" : ""}`}>
          ...
          {!selectedGroupMemberId && <p className="text-sm text-amber-600">Please select a member to settle with</p>}
        </div>
                  ...
                  <Label htmlFor="theyPaid" className="flex-grow cursor-pointer">
                    <div className="flex items-center">
                      <Avatar className="h-6 w-6 mr-2">
                        ...
                        <AvatarFallback>{groupMembers.find((m) => m.userId === selectedGroupMemberId)?.name.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <span>{groupMembers.find((m) => m.userId === selectedGroupMemberId)?.name} paid you</span>
                    </div>
                  </Label>
                </div>
              </RadioGroup>
        ...
        <Button type="submit" className="w-full" disabled={isSubmitting || !selectedGroupMemberId}>
          {isSubmitting ? "Recording..." : "Record settlement"}
        </Button>
      </form>
    );
